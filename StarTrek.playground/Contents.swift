...
let female = humanOfficers.filter{femaleOfficers.contains($0)}.count

let percentage = Float(female)/Float(humanCount)
...
let list = femaleOfficers.sorted()
...
let allCaptians: [Officer] = captains.map{Officer(name: $0, ship: $1)}
print(allCaptians.map{$0.name})


// Q5: Alphabetize the previous list by the captain's name
let allCaptiansAlpha = captains.map{Officer(name: $0, ship: $1)}.sorted{$0.name < $1.name}

print(allCaptiansAlpha.map{$0.name})
...
let vulcans: [Officer] = firstOfficers.map{Officer(name: $0, ship: $1)}.filter{vulcanOfficers.contains($0.name)}
...
let countVulcan = vulcans.filter{$0.name.count < 5}.count
